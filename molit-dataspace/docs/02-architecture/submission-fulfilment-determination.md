...
작성일: 2026-08-03  
작성 기준: 2026-08-03  
관련 결정: `E-12`, `E-15`, `E-16`, `E-19`, `E-20`  
...
- **(Decision)** 데이터 스페이스는 신원·카탈로그·계약·정책·감사를 담당하며 payload를 보관하거나 중계하지 않는다.
  - **(경계)** 실제 바이트는 원천에서 Consumer로 직접 이동한다.
...
Consumer가 승인된 계약 범위에서 원천을 직접 PULL하면 실제 바이트를 받을 수 있는 상태여야 한다.
...
| 4 | Provider transfer worker의 접근 자원 발급과 `active` | token 또는 signed URL 발급, Connector 접수 | 불가 — 실제 바이트 읽기를 입증하지 않음 |
| 5 | 계약 scope의 직접 PULL 성공 | 원천 응답 byte·Content-Type·digest와 계약·Transfer 결속 | 수신 가능 후보 — 시험 행위자 선택은 미정 |
| 6 | Consumer 내부 적재·업무처리 성공 | Consumer 자체 시스템의 후속 처리 | 수신 가능 범위 초과 후보 — 기술적 온보딩·업무검증과 경계 설정 필요 |
...
- **(한계)** `active`만으로 Consumer의 실제 byte 읽기 성공을 증명할 수 없다.
- **(책임 경계)** worker는 실제 byte를 읽지 않는다. 승인된 시험 Consumer 또는 지정 Consumer가 발급된 접근 자원으로 원천에서 직접 PULL한다.
- **(보안 경계)** token, signed URL과 `DataAddress` 원문은 제출 판정 기록이나 일반 감사로그에 저장하지 않는다.
  - **(대체 증거)** 발급 결과 ID·digest, 계약·Transfer 상관키, 유효기간, 원천 응답 결과와 폐기 receipt를 사용한다.
...
| --- | --- | --- | --- |
| Provider 기능 수행 주체 통제 | Offering 유효성, source binding, 승인된 PULL 처리, 접근 자원 발급·scope·만료·회수, 원천 가용성 | 게시 상태, binding digest, worker journal, 발급 receipt, 동일 경로 probe 결과 | 제공자 측 준비 여부를 분리 가능 |
| Consumer 기능 수행 주체 통제 | 기관 신원, 계약·Transfer 요청, client credential, 네트워크·저장용량, 내부 수신 처리 | Consumer 요청·응답 log, 수신 digest, 오류 code, 내부 readiness 결과 | Consumer 미준비와 제공자 미이행을 분리할 필요 |
...
다음 선택지는 미채택 상태다. 이 문서에서 우선순위를 정하지 않는다.

| 선택지 | 수신 가능 판정 지점 | 귀결 | 추가 승인 필요 |
| --- | --- | --- | --- |
| Provider 통제 범위 기준 | 운영자가 승인한 시험 Consumer가 접근 자원 발급 뒤 같은 scope로 원천에서 직접 PULL | 지정 Consumer 미준비로 제공자 판정이 바뀌지 않지만 실제 지정 Consumer의 수신능력을 입증하지 못함 | 시험 Consumer·경로 동등성·증거 유효기간 |
| 지정 Consumer 종단 기준 | 기존 정산 시스템이 계약된 원천에서 실제 PULL에 성공 | 실제 수신 증거가 강해지지만 Consumer 장애·미준비가 제공자 미이행으로 귀속될 수 있음 | 장애 귀속·재시험·유예 규칙 |
| 양측 하위상태 분리 | Provider 준비와 Consumer 준비를 별도 기록하고 법적 결합 규칙으로 수신 가능 여부 산출 | 원인과 책임을 분리할 수 있으나 상태·감사 복잡도가 늘어남 | 하위상태 의미와 법적 결합 규칙 |

수신 가능 상태의 최종 판정 지점과 Consumer 개입 범위는 `OPEN-SFD-02`에서 결정한다.
...
| 전송 | 시험 Agreement를 `FINALIZED`한 뒤 지정 시험 Consumer가 승인된 PULL로 원천의 실제 byte를 읽음 | 계약·Transfer·원천 응답이 같은 실행 ID에 결속되고 byte 수·Content-Type·digest가 기대 결과와 일치 | 협상 PID, Agreement ID, Transfer PID, 발급 receipt digest, 원천·Consumer log, byte 수·Content-Type·payload digest |
| 정정 | 통제된 오류 version을 원천 정정 경로로 수정하고 새 Offering version을 재게시한 뒤 다시 PULL | 정정 요청·원천 version·게시 version이 순서대로 연결되고 새 version이 조회·수신되며 이전 version 처리 결과가 기록됨 | 정정 요청, 전후 version·digest, SHACL report, Catalog diff, 재PULL 결과, 이전 version 상태 사건 |
...
실제 원천 결속, SHACL report와 실제 PULL 결과는 서로 다른 증거로 보존한다.
...
| 수신 가능 | 미판정, 준비 중, 가능, 불가능, 일시 중단, 종료, 재시험 필요 | 접근 자원 발급, PULL probe, 원천·Consumer 장애, 회수·복구 | worker·원천·Consumer 사건과 책임 구분 |
...
| 전송·시험 | Transfer PID, 외부 자원 ID, source request 상관키, test run ID, 시험 기준 version | token·signed URL·`DataAddress` 원문 제외 |
...
| --- | --- | --- |
| 감사 | 제출 의무·기간, 세 조건 판정, Agreement·Offering version, PULL·온보딩 실행, 판정 주체와 무결성 receipt | payload 내용과 정산 계산의 진실성 자체를 증명하지 않음 |
...
- **(Inferred — DRV-04)** 온보딩 미완료 참가자는 DS 경유 이행이 성립하지 않으므로 **기존 경로 병행이 필수**. 전환 일정(`E-15`)은 온보딩 완료율에 종속
  - **(해석 경계)** DS 경유 이행은 신원·카탈로그·계약·정책·감사 절차를 거친 이행이며 payload 경유를 뜻하지 않는다.
...
| `OPEN-SFD-02` | `DRV-03`·`T-06` | `Unverified` | 수신 가능 상태의 판정 지점과 Consumer 준비 개입 범위 | Consumer 미준비를 Provider 미이행으로 볼지 판정 불가 | 미정 | 미정 | 선택지 채택, 장애 귀속·재시험·증거 기준 승인 |
...
| `OPEN-SFD-08` | `T-06` | `Verified` | 전송 토폴로지가 문서마다 다르다. [아키텍처 §5](edc-caas-dsaas-architecture.md)는 Provider transfer worker의 비범위를 `데이터 바이트 프록시`로 두고 §6은 원천 플랫폼 token·signed URL 발급으로 규정하나, [목표 아키텍처](target-architecture.md)와 [Offering 수명주기](offering-onboarding-lifecycle.md)의 순서도는 `Consumer → Data Plane → 원천` 경로로 그린다 | 수신 가능 상태의 판정 지점과 감사 기록의 관측점이 토폴로지에 따라 달라져 조건 2를 확정할 수 없음 | 미정 | 미정 | 두 토폴로지 중 채택안 결정 또는 병존 조건 확정 후 관련 도식 정렬 |
