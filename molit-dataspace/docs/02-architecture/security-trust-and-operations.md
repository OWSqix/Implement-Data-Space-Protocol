...
경로 기반 참가 등급과 관리형·자체 운영 경로의 판정 정본은 [참가자 온보딩·보증 설계](participant-onboarding-and-assurance.md)다.
...
공통 감사 ID는 현행 식별자 목록에 포함하지 않고 다음 후보 표에서 관리한다. 서로 의미가 다른 식별자의 혼합 여부는 [EDC 기반 CaaS·DSaaS 구성 설계](edc-caas-dsaas-architecture.md) §3.1의 식별자 분리 원칙으로 판정한다.

| 후보 식별자 | 상태 | 준거 |
| --- | --- | --- |
| 공통 감사 ID | 후보 — ADR-0006 Proposed | EDC 기반 CaaS·DSaaS 구성 설계 §3.1의 식별자 분리 원칙 |
...
다음 표는 거래 유형별 증적 요구의 판정 기준이다. 중앙 기록 의무의 규범 문장은 [ADR-0006](../adr/0006-selective-notary-evidence.md)(Proposed — 승인 전 규범 아님)에 있다.

| 거래 유형(예시) | 판정 축 ①: 제3자 지급 존재 | 판정 축 ②: 보조금·감사 규정 해당 | 판정 결과(ADR-0006 제안 기준) | 판정을 확정할 증거 |
| --- | --- | --- | --- | --- |
| 준공영제 운행·수입 데이터 제출 | 있음 — 지자체 재정지원 정산 | 해당 추정 — 조례·표준 협약 | 중앙 notary 기록 대상 | 지자체 조례·협약의 정산 증빙 조항 |
| 공공 발주 BIM 산출물 인도 | 있음 — 발주청 대금 지급 | 해당 추정 — 계약·감사 규정 | 중앙 notary 기록 대상 | 발주 계약 조항, 감사 규정 조문 |
| 건설사 간 자재 주문·송장 교환 | 없음 — 양자 상거래 | 비해당 | 참여자 로컬 감사 로그 + 공통 감사 ID | — |
| 판정 축 미확인 거래 | 미확인 | 미확인 | 판정 보류 — 로컬 로그 + 감사 ID(fail-closed) | 법무 검토(R-053) |

다음 소표는 거래유형 판정표의 열을 바꾸지 않고 ADR-0006 재검토 축의 확정·미확정 상태를 기록한다.

| 비교 축 | 2026-08-01 확인값 | 배치 판정 영향 |
| --- | --- | --- |
| record 스키마 | 서명 hash·시각·공통 감사 ID만 포함, 당사자·자산 식별정보 제외 | 축 충족; schema artifact·version은 구현 시 고정 |
| 열람 절차 | 정기 정산 사전 동의 범위 자동 제출, 범위 밖·감사·분쟁은 건별 법적 근거 확인 | 절차 정의; 주체별 권한 미정 |
| 민감도 | 필드별 등급 미확정 | 배치 미선택 유지 |
| 보존 | 원 record·개봉·대사·backup 범위 미확정 | 배치 미선택 유지 |
| 운영 주체 | 법인·책임 경계 미확정 | 배치 미선택 유지 |

표의 추정 항목과 거래 유형별 법적 근거는 `Unverified`다. 거버넌스·법무 담당은 지자체 조례·협약, 발주 계약과 감사 규정 조문을 대조하고 결과를 [R-053](../03-plan/risk-register.md)에 기록한다.
