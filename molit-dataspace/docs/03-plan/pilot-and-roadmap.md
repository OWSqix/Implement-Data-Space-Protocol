...
작성일: 2026-07-11  
작성 기준: 2026-08-06 결정 인터뷰 `E-22`~`E-27`과 2026-08-03 저장소 정본  
최종 개정: 2026-08-06  
상태: Draft
관련 결정: `E-15`, `E-16`, `E-17`, `E-18`, `E-19`, `E-20`, `E-21`, `E-22`, `E-23`, `E-24`, `E-25`, `E-26`, `E-27`
...
| `E-22` | 파일럿 도시 **우선 후보**로 청주시를 기록한다. **잠정 기록이며 선정 확정이 아니다.** 단계 0 도구를 청주시에 우선 적용하되 청주시는 기초지자체라 충청북도 층위가 추가돼 전 항목을 신규 판정한다 |
| `E-23` | `A-04`의 **우선 후보**를 후보 3(ITS 표준 노드·링크 파일 snapshot)으로 지정한다. **우선 후보 지정일 뿐이며 착수·확정은 확장 단계 결정과 `G0~G6` 통과를 조건**으로 한다 |
| `E-24` | `DEF-05` 정산 payload는 **범주 골격만** 승인한다. 승인 범위는 운행 실적·수입금 집계·비용 항목·대사 결과·공통 감사 ID이며 **필드 수준은 미결정**이다 |
| `E-25` | `DEF-06`은 **기준선 측정을 먼저 하고 측정 설계를 승인**한다. **수치 목표는 기준선 확정 뒤에 정하며** 이 단계에서 목표값을 두지 않는다 |
| `E-26` | `DEF-01`·`DEF-02`는 **KAIA 위임을 전제**로 진행한다. 분야 DSGA = KAIA는 **잠정 배정이고 근거가 구두 전언이라 `Unverified`**다. `DEF-03`은 착수하지 않고 `OPEN-TOP-06`을 유지한다 |
| `E-27` | `DEF-07`의 **재평가 주기는 연 1회로 확정**한다. **자립 전환 연한은 파일럿과 함께 정하므로 미정으로 유지**하고 `DEF-04`·`DEF-08`은 유보를 유지한다 |

`E-22`~`E-27`은 2026-08-06 결정 인터뷰 결과다. `E-22`·`E-26`은 잠정이고 `E-23`은 조건부이므로 이 표의 등재만으로 도시 선정·`A-04` 착수·기관 배정이 확정되지 않는다. 기준일: 2026-08-06.

`E-26`의 KAIA 배정은 구두 전언을 근거로 한 잠정 배정이며 공식 위임 문서는 미확인이다. 공식 문서 확인은 `OPEN-TOP-06`으로 유지한다.
...
7. **(Decision — E-22)** 단계 0의 자료 확보 목록·질문서·기준선 조사 도구를 우선 후보 청주시에 먼저 적용한다. 우선 후보는 잠정이며 도시 선정 확정이 아니다.
8. 청주시는 기초지자체이므로 문서 층위 지도에 충청북도(광역)와 청주시(기초)의 위임 관계를 추가해 재구성한다. 적용 대상 목록은 [파일럿 권리 확정 패키지 §2.1](pilot-rights-confirmation-package.md#21-문서-확보-gate)을 사용한다.
...
- 우선 후보 청주시의 문서 층위 지도에 광역·기초 위임 관계가 별도 행으로 구분됨
...
| 순서 | 후보 | 사용 목적 | 현재 상태 |
| --- | --- | --- | --- |
| 1 | 파일럿 운수사 원천의 승인된 정산 Dataset | 허브 없는 Provider Data Plane 경유 PULL full lifecycle | 우선 후보 청주시(`E-22`, 잠정)이며 선정 확정 아님. 조례·협약·위수탁계약·데이터 권리 증거 필요 |
| 2 | 분석 데이터셋 metadata `GET` | Discovery Bridge | 설계 가능, endpoint·HTTPS 확인 전 실행 차단 |
| 3 | ITS 표준 노드·링크 파일 | finite snapshot fallback | `A-04` 우선 후보(`E-23`, 잠정). 착수는 확장 단계 결정과 `G0~G6` 통과 조건이며 권리·source 계약 필요 |
...
우선 후보 청주시는 기존 6개 조사 도시 밖이므로 전 항목을 신규 판정한다. 청주시는 기초지자체라 광역·기초 위임 층위가 추가되어 선행조건의 성격이 6개 광역시와 다르다.
근거: [파일럿 권리 확정 패키지 §2.1](pilot-rights-confirmation-package.md#21-문서-확보-gate), 기준일: 2026-08-06.
...
| `RULE-04` | 우선 후보 청주시(`E-22`, 잠정)의 조례·시행규칙·정산지침 개정. 조례·시행규칙 원문은 확보했고 정산지침 원문과 개정 권한이 `Unverified` | 현행 원문·개정 권한 확인과 공통 완료조건 충족 | 미정 | 미정 | [준공영제 정산 데이터 필드 실물 조사 §5](../01-research/settlement-data-field-survey.md#5-청주시-준공영제-팩트시트), 기준일: 2026-08-06 |
| `AGR-05` | 청주 시행 협약과 충청북도–청주시 위임 관계 문서가 `Unverified`. 협약 원문은 미확보 | 협약 원문·부속합의서 확보와 광역·기초 위임 경로 확인 | 미정 | 미정 | [같은 조사 §6](../01-research/settlement-data-field-survey.md#6-충청북도-청주시-권한-관계), 기준일: 2026-08-06 |

`RULE-04`·`AGR-05`는 기초지자체 선행조건이므로 6개 광역시를 대상으로 한 `RULE-02`·`AGR-03`과 성격이 다르다. 광역(충청북도) 위임 층위가 하나 더 붙는다.
...
`RULE-04`·`AGR-05`의 한계는 다음과 같다.

- **(Unverified)** 청주시 정산지침·운영지침과 그 개정 권한은 문서 미확인. 조례·협약의 확인 범위와 근거 등급은 [준공영제 정산 데이터 필드 실물 조사 §5](../01-research/settlement-data-field-survey.md#5-청주시-준공영제-팩트시트)를 정본으로 봄
- **(Unverified)** 청주 「시내버스 준공영제 시행 협약」 원문은 미확보이며 언론 보도 본문만 확인됨 — `OPEN-SFS-03`, 기준일: 2026-08-06
- **(Unverified)** 충청북도에서 청주시로 이어지는 시내버스 면허·사무 위임의 법적 경로는 확정하지 못함 — `OPEN-SFS-02`, 기준일: 2026-08-06
...
| 2026-08-06 | 2026-08-06 결정 6건이 저장소 어디에도 등재되지 않아 다른 문서의 인용에 도착 지점이 없었음 | §2.2 확정 결정 표에 6건을 등재하고 각 결정문에 잠정·조건부·부분 확정 성격을 명시 | `E-22`~`E-27` |
| 2026-08-06 | 파일럿 도시와 `A-04` 우선 후보가 본문에 없었음 | §4.1 작업에 청주시 우선 적용 항목을, §5.1 후보 표에 청주시와 `A-04` 우선 후보 표기를 추가 | `E-22`, `E-23` |
| 2026-08-06 | 외부 선행조건이 6개 광역시만 대상이라 우선 후보 도시의 선행조건이 누락 | §14.2에 `RULE-04`·`AGR-05`를 `Unverified`로 신규 등록하고 광역·기초 위임 층위 차이를 명시 | `E-22` |
...
| `OPEN-PRM-08` | `RULE-04` | `Unverified` | 우선 후보 청주시의 현행 정산지침과 개정 권한 미확보. 조례·시행규칙 원문은 확보 | 청주 대상 제출 인정·장애·정정 효과 판정 불가 | 미정 | 미정 | 현행 원문·개정 권한·승인본과 공통 완료조건 증거 확보 |
| `OPEN-PRM-09` | `AGR-05`, `OPEN-SFS-02`, `OPEN-SFS-03` | `Unverified` | 청주 시행 협약 원문과 충청북도–청주시 위임 관계 미확인 | 우선 후보 도시의 권리·책임분담과 면허 소재 판정 불가 | 미정 | 미정 | 협약 원문·부속합의서와 위임 경로 근거 조문 확보 |
