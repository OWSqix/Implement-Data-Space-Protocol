...
개정 기준: `E-21`·`E-22`·`E-23`·`E-24`·`E-25`  
관련 결정: `E-16`, `E-17`, `E-19`, `E-20`, `E-21`, `E-22`, `E-23`, `E-24`, `E-25`  
상태: Draft
...
기준선 실측 선례·측정 방법론·EU 파일럿 KPI 사례는 [KPI 기준선 선례 조사](../01-research/kpi-baseline-precedents.md)를 참조한다.

- **(Decision — E-25)** DEF-06은 **기준선 측정을 우선**하고 측정 설계를 승인한다. 수치 목표는 기준선 확정 뒤에 후행한다 — 근거: `E-25`, 기준일: 2026-08-06
...
- **(현재 상태)** 후보 1은 "대상 미식별·운영기관 증거 대기" 상태이며 확장 단계 후보다.
...
`A-04`는 **초기 출범을 막는 차단이 아니라 확장 단계 준비를 막는 차단**으로 재분류한다. 후보 확정은 **기존 공개 플랫폼 연계 착수 시점**으로 이연한다.

- **(Decision — E-23)** 우선 후보는 후보 3(ITS 표준 노드·링크 파일 snapshot)으로 지정한다(잠정) — 근거: `E-23`, 기준일: 2026-08-06
  - 착수와 확정 기록은 연계 착수 시점의 `G0~G6` 통과를 조건으로 유지한다. 지정 자체는 착수가 아니다.
- **(상태)** 후보 6개와 후보 1의 문서상 상태는 `Verified`이고 `E-17` 적용과 `A-04` 재분류·이연은 `Decision`이다.
  - 후보 3 우선 지정도 `Decision`(`E-23`)이며 착수 전 `G0~G6` 판정은 `Unverified`다.
...
10. `A-04` 우선 후보(후보 3) 지정이 초기 출범 차단이나 `G0~G6` 통과로 집계되지 않음
...
| `OPEN-IUK-05` | `A-04` | `Decision` | 우선 후보를 후보 3(ITS 표준 노드·링크 파일 snapshot)으로 지정(`E-23`, 2026-08-06, 잠정). 착수·확정은 기존 공개 플랫폼 연계 착수 시점의 G0~G6 통과 조건 | 초기 출범은 차단하지 않고 확장 단계 준비를 차단 | 프로젝트 의뢰자 | 확장 단계 착수 시 | 착수 시 후보 3의 G0~G6 판정과 확정 기록 기입 |
| `OPEN-IUK-06` | `DEF-05` | `Unverified` | 정산 payload 필드와 첫 실증 자산 범위 미확정. 범주 골격은 2026-08-06 승인(`E-24`) | 정산 명세의 필드 수준 계약시험 판정 불가 | 분야 DSGA(KAIA 잠정 지명, `Unverified` — [거버넌스 원칙 §6](../02-design/governance-and-operating-principles.md#6-미결정-주체와-법문화) 결정 기록 참조) | 미정 | 리서치·실물 조사(우선 후보 청주, 잠정)로 필드 확정 후 payload 범위 승인 |
| `OPEN-IUK-07` | `DEF-06` | `Unverified` | KPI 수치 목표와 기준기간 미결정. 측정 설계는 2026-08-06 승인(`E-25`) | 목표 대비 성과 판정 불가 | 분야 DSGA(KAIA 잠정 지명, `Unverified` — [거버넌스 원칙 §6](../02-design/governance-and-operating-principles.md#6-미결정-주체와-법문화) 결정 기록 참조) | 미정 | 기준선 측정 우선, 수치 목표는 리서치·실증 착수 후 변경 기록으로 승인 |
...
| 2026-08-06 | §6.1이 A-04 후보 미선택 전제였고 §7 검수 10항도 같은 전제 | 우선 후보를 후보 3으로 지정한 결정문을 §6.1에 넣고 검수 10항을 지정 이후 기준으로 고침. "우선순위 1" 표현을 후보 번호 표기로 정렬 | `E-23` |
| 2026-08-06 | §5가 측정 설계 승인을 각주로만 언급 | DEF-06 기준선 측정 우선과 측정 설계 승인을 §5 결정문으로 승격하고 `OPEN-IUK-07`에 결정 ID를 연결 | `E-25` |
| 2026-08-06 | `OPEN-IUK-06`이 범주 골격 승인을 결정 ID 없이 기록 | 범주 골격 승인에 결정 ID를 연결하고 실물 조사 우선 도시가 잠정임을 유지 | `E-22`·`E-24` |
| 2026-08-06 | `OPEN-IUK-06`·`OPEN-IUK-07` 담당이 라벨 없이 "(잠정)"만 표기 | 담당을 "분야 DSGA(KAIA 잠정 지명, `Unverified`)"로 통일하고 거버넌스 원칙 §6 결정 기록으로 링크 | `E-26` |
