...
- **(상태)** 후보 6개와 우선순위 1의 문서상 상태는 `Verified`, `E-17` 적용과 `A-04` 재분류·이연은 `Decision`, 후보 미선택은 `Unverified`
...
| `OPEN-IUK-05` | `A-04` | `Unverified` | Platform Bridge 후보의 Candidate ID·결정자·결정일 미정. 후보 선택은 기존 공개 플랫폼 연계 착수 시점으로 이연 | 초기 출범은 차단하지 않고 확장 단계 준비를 차단 | 미정 | 미정 | 기존 공개 플랫폼 연계 착수 시 후보의 G0~G6 판정과 선택 승인 기록 확보 |
| `OPEN-IUK-06` | `DEF-05` | `Unverified` | 정산 payload 필드와 첫 실증 자산 범위 미확정 | 정산 명세의 필드 수준 계약시험 판정 불가 | 미정 | 미정 | 실물 조사와 분야 DSGA의 payload 범위 승인 |
| `OPEN-IUK-07` | `DEF-06` | `Unverified` | KPI 수치 목표와 기준기간 미결정 | 목표 대비 성과 판정 불가 | 미정 | 미정 | 기준선 측정과 수치 목표·기준기간 변경 기록 승인 |
