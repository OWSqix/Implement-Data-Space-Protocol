...
권고 property와 deprecated URI 검사는 `publication-policy`에서 별도 실행한다. 0.1.0의 `core-publication`과 `geo-publication` profile 이름은 RC.1에서 제거했다. `publish-check`가 선택한 conformance module 뒤에 동일한 publication policy를 적용한다.
...
- `withheld` 공개수준이면 location·bbox·centroid·geometry serialization 게시 금지
- 공간 해상도는 미터 단위 양수 값으로 제한

원 데이터의 `referenceSystem`과 검색용 geometry literal의 CRS는 별도 의미다. 두 값의 일치를 강제하지 않는다.

`exact`는 RC.1의 후보 통제값이므로 SHACL 적합만으로 보안 검토 승인을 뜻하지 않는다. 실제 exact geometry 게시는 별도 보안·개인정보 검토에서 허용된 Dataset에 한정한다.
